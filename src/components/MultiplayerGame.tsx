...
export const MultiplayerGame: React.FC<MultiplayerGameProps> = ({ socket, onLeave }) => {
  ...
  return (
    <div className={styles.layout}>
        ...
        {gameState.players.map((player: MultiplayerPlayerView, index: number) => {
          ...
          return (
            ...
            >
              ...
              <span className={scoreStyles.aiIndicator}>{player.cardCount} cards</span>
            </div>
          )
        })}
      </div>
      ...
      {/* Deck count */}
      <div className={styles.hintBadge}>Deck: {gameState.deckCount}</div>
      ...
      <div className={styles.boardArea}>
        <BoardComponent
          ...
          selectedCard={
            isMyTurn && gameState.pendingPlacements.length < MAX_LINE_LENGTH ? selectedCard : null
          }
          zoomLevel={zoomLevel}
          ...
          >
            ...
            {[...gameState.players]
              ...
              .map((p, i) => (
                <div
                  key={p.id}
                  style={{ padding: "8px 0", display: "flex", justifyContent: "space-between" }}
                >
                  <span>
                    {i === 0 ? "🏆 " : ""}
                    {p.name}
