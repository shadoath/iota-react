...
describe('RoomManager', () => {
  ...
  describe('startGame', () => {
    ...
    it('deals 4 cards to each player', () => {
      ...
      const aliceView = manager.getGameStateForPlayer(code, manager.getPlayerIdBySocket(code, 'socket-1')!)
  ...
  describe('game actions', () => {
    ...
    it('placeCard validates placement', () => {
      const view = manager.getGameStateForPlayer(code, manager.getPlayerIdBySocket(code, 'socket-1')!)
    ...
    it('rejects actions from wrong player', () => {
      const view = manager.getGameStateForPlayer(code, manager.getPlayerIdBySocket(code, 'socket-2')!)
      if (view?.hand[0]) {
