...
export function analyzeGame(
  ...
  for (let i = 0; i < turnHistory.length; i++) {
    ...
    if (player?.type === "human" && turn.placements.length > 0) {
      ...
      const placedCardIds = new Set(turn.placements.map((p) => p.card.id.replace("pending-", "")))
      ...
      const rating: TurnAnalysis["rating"] = diff <= 0 ? "optimal" : diff <= 2 ? "good" : "missed"
    ...
    if (playerIndex >= 0) {
      const playedIds = new Set(turn.placements.map((p) => p.card.id.replace("pending-", "")))
